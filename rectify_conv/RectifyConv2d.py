...
class RectifyConv2d(_ConvNd):
    ...
    def __init__(
        ...
        self.offsetfield[0::2, :, :] = distortion_map[
            distortion_map.shape[0]//2 - kernel_size[0]//2 : distortion_map.shape[0]//2 + kernel_size[0]//2 + 1,
            distortion_map.shape[1]//2 - kernel_size[1]//2 : distortion_map.shape[1]//2 + kernel_size[1]//2 + 1,
            0, :, :].reshape(-1, distortion_map.shape[-2], distortion_map.shape[-1])
        self.offsetfield[1::2, :, :] = distortion_map[
            distortion_map.shape[0]//2 - kernel_size[0]//2 : distortion_map.shape[0]//2 + kernel_size[0]//2 + 1,
            distortion_map.shape[1]//2 - kernel_size[1]//2 : distortion_map.shape[1]//2 + kernel_size[1]//2 + 1,
    ...
    def calculate_output_size(self, input_size):
        # Use the standard conv2d output formula:
        # out = floor((input + 2*pad - dilation*(kernel-1) - 1) / stride) + 1
        h = math.floor((input_size[0] + 2 * self.padding[0] - self.dilation[0]*(self.kernel_size[0]-1) - 1) / self.stride[0]) + 1
        w = math.floor((input_size[1] + 2 * self.padding[1] - self.dilation[1]*(self.kernel_size[1]-1) - 1) / self.stride[1]) + 1
        return [h, w]

    def configure_offset(self, input_size):
        output_size = self.calculate_output_size(input_size[-2:])
        offsetfield = F.interpolate(
            self.offsetfield.unsqueeze(0),
            size=(output_size[0], output_size[1]),
            mode="bilinear",
            align_corners=True,  # use True to align grid coordinates exactly
        )
